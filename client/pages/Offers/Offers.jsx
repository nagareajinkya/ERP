               ...
               <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 rounded-xl shadow-xl max-h-48 overflow-y-auto z-50">
                 {filtered.map(product => (
                   <div key={product.id} onClick={() => { onSelect(product); setIsOpen(false); }} className="px-4 py-2 hover:bg-green-50 cursor-pointer text-sm text-gray-700 flex justify-between group">
                     ...
                     <span className="text-gray-400 text-xs group-hover:text-green-600 bg-gray-50 px-2 py-0.5 rounded font-medium">{product.unit}</span>
                   </div>
                 ))}
               </div>
             )}
...
const Offers = () => {
  ...
  const initialForm = {
    name: '',
    ruleType: 'cart_value', 
    buyProductName: '', buyProductUnit: 'kg', buyQty: '1',
    getProductName: '', getProductUnit: 'kg', getQty: '1',
  ...
  const [offers, setOffers] = useState([
    { id: 1, name: 'Diwali Dhamaka', type: 'cart_value', rule: 'Cart Discount', value: '10%', status: 'active', usage: 42, start: '2025-10-20', end: '2025-11-05', target: 'All Customers', usageType: 'unlimited', color: 'bg-blue-50 text-blue-600', desc: 'Flat 10% Off on Bill > ₹1000', 
      ruleType: 'cart_value', discountType: 'percentage', discountValue: '10', minPurchase: '1000', targetType: 'all', startDate: '2025-10-20', endDate: '2025-11-05', selectedCustomers: []
    },
    { id: 2, name: 'Buy Sugar Get Salt Free', type: 'bogo', rule: 'Free Item', value: '1 Free', status: 'active', usage: 15, start: '2025-10-01', end: '2025-10-31', target: 'Top 50 Spenders', usageType: 'single', color: 'bg-purple-50 text-purple-600', desc: 'Free Salt with Sugar',
      ruleType: 'bogo', buyProductName: 'Sugar', buyProductUnit: 'kg', buyQty: '1', getProductName: 'Tata Salt', getProductUnit: 'kg', getQty: '1', targetType: 'top_spenders', topSpenderCount: '50', startDate: '2025-10-01', endDate: '2025-10-31', selectedCustomers: []
    },
    { id: 3, name: 'Monsoon Sale', type: 'product_disc', rule: 'Product Discount', value: '₹50 Off', status: 'expired', usage: 120, start: '2025-06-01', end: '2025-06-30', target: 'Specific', usageType: 'unlimited', color: 'bg-orange-50 text-orange-600', desc: '₹50 Off on Rice',
      ruleType: 'product_disc', buyProductName: 'Basmati Rice', buyProductUnit: 'kg', buyQty: '1', discountType: 'flat', discountValue: '50', targetType: 'specific', startDate: '2025-06-01', endDate: '2025-06-30', selectedCustomers: [102, 104]
    }
  ]);

  // --- DYNAMIC ANALYTICS CALCULATIONS ---
  const activeOffersCount = offers.filter(o => o.status === 'active').length;
  const totalRedeemed = offers.reduce((sum, offer) => sum + (offer.usage || 0), 0);
  const totalDiscountValue = totalRedeemed * 355; // Using approx ₹355 saved per redemption for prototype
  ...
  const handleSubmit = (e) => {
    ...
    const newOffer = {
      ...
      rule: formData.ruleType === 'cart_value' ? 'Cart Discount' : formData.ruleType === 'bogo' ? 'Free Item' : 'Product Discount',
      desc: formData.ruleType === 'cart_value' ? `${formData.discountType === 'percentage' ? formData.discountValue + '%' : '₹' + formData.discountValue} Off on Bill > ₹${formData.minPurchase}` 
          : formData.ruleType === 'product_disc' ? `${formData.discountType === 'percentage' ? formData.discountValue + '%' : '₹' + formData.discountValue} Off on ${formData.buyProductName}`
          : `Buy ${formData.buyQty} ${formData.buyProductName}, Get ${formData.getQty} ${formData.getProductName} Free`,
      value: formData.ruleType === 'bogo' ? `${formData.getQty} Free` : `${formData.discountValue}${formData.discountType === 'percentage' ? '%' : '₹'}`,
      ...
      color: formData.ruleType === 'bogo' ? 'bg-purple-50 text-purple-600' : formData.ruleType === 'cart_value' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'
    };
    
    if(editingOfferId) setOffers(offers.map(o => o.id === editingOfferId ? newOffer : o));
    else setOffers([newOffer, ...offers]);
            ...
            <div className="p-5">
              <div className="flex justify-between items-start mb-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${offer.color}`}>
                  {offer.type === 'bogo' ? <Gift size={20}/> : offer.type === 'cart_value' ? <Percent size={20}/> : <Tag size={20}/>}
                </div>
              ...
              <p className="text-sm font-medium text-gray-500 mb-4">{offer.desc}</p>
              
              <div className="flex flex-col gap-2">
                 <div className="flex items-center gap-2 text-xs font-bold bg-gray-50 p-2 rounded-lg text-gray-600"><Users size={14} className="text-blue-500"/><span>{offer.target}</span></div>
                 <div className="flex items-center gap-2 text-xs font-bold bg-gray-50 p-2 rounded-lg text-gray-600"><Receipt size={14} className="text-orange-500"/><span>Used {offer.usage} times</span></div>
              </div>
            </div>
                            ...
                            <div key={cust.id} onClick={() => toggleCustomerSelection(cust.id)} className={`flex items-center justify-between p-3 bg-white border rounded-xl cursor-pointer transition-all ${formData.selectedCustomers.includes(cust.id) ? 'border-green-500 ring-2 ring-green-100 bg-green-50/50' : 'border-gray-100 hover:bg-gray-50'}`}>
                               <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                     ...
                                     <p className="text-xs text-gray-500 font-medium">Spent: {cust.spend} • {cust.visits} Visits</p>
                                  </div>
                               </div>
                               <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${cust.type === 'Top Spender' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'}`}>{cust.type}</span>
                            </div>
                          ))}
                       </div>
                    </div>
                 ...
                 <div>
                    ...
                    <div className="border border-gray-100 rounded-xl overflow-hidden shadow-sm">
                       <table className="w-full text-left text-sm">
                          ...
                          <tbody className="divide-y divide-gray-100 font-medium">{viewOffer.usage > 0 ? REDEMPTIONS_DATA.map((r) => (<tr key={r.id} className="hover:bg-gray-50/50"><td className="p-4 text-gray-800 font-bold">{r.customer}</td><td className="p-4 text-gray-500">{r.date}</td><td className="p-4 text-gray-500">{r.count} times</td><td className="p-4 text-right font-extrabold text-green-600">{r.save}</td></tr>)) : (<tr><td colSpan="4" className="p-8 text-center text-gray-400">No redemptions yet.</td></tr>)}</tbody>
                       </table>
                    </div>
                 </div>
              </div>
