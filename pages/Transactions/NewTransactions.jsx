...
import { 
  ...
  Search, Zap, CheckCircle2, X, Gift, History // <--- Imported History Icon
} from 'lucide-react';

// --- MOCK DATA (Sorted Alphabetically) ---
const PRODUCTS = [
...
const NewTransaction = ({ type = 'sale' }) => {
  ...
  const [items, setItems] = useState([{ id: Date.now(), name: '', qty: 1, price: '', amount: 0, isFree: false, manual: false }]);
  ...
  const handleAddItem = () => {
    ...
    if (lastItem && lastItem.name === '' && !lastItem.isFree) {
        if (itemRefs.current[`name-${lastItem.id}`]) itemRefs.current[`name-${lastItem.id}`].focus();
        return;
    }
    const newId = Date.now();
    setItems(prev => [...prev, { id: newId, name: '', qty: 1, price: '', amount: 0, isFree: false, manual: false }]);
    setTimeout(() => { if (itemRefs.current[`name-${newId}`]) itemRefs.current[`name-${newId}`].focus(); }, 50);
  };

  const handleRemoveItem = (id, isOfferRemoval = false) => {
    if (isOfferRemoval) {
        const item = items.find(i => i.id === id);
        if (item && item.offerId) setRemovedOfferIds(prev => [...prev, item.offerId]);
        setItems(prev => prev.filter(i => i.id !== id));
    } else {
        if (items.length === 1 && !items[0].isFree) {
            // Clear last row
            setItems([{ id: Date.now(), name: '', qty: 1, price: '', amount: 0, isFree: false, manual: false }]);
        } else {
            setItems(prev => prev.filter(i => i.id !== id));
        }
    }
  ...
  const handleUpdateItem = (id, field, value) => {
    setItems(prev => prev.map(item => {
      if (item.id === id) {
        if (field === 'qty' && value < 1) return item; 
        const updatedItem = { ...item, [field]: value };
        if (item.isFree) updatedItem.manual = true; // Mark as manually edited
  ...
  const handleProductSelect = (rowId, product) => {
    ...
    setTimeout(() => { if (itemRefs.current[`qty-${rowId}`]) itemRefs.current[`qty-${rowId}`].focus(); }, 50);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm min-h-[400px] flex flex-col">
            <div className="flex justify-between items-center mb-4"><h3 className="font-semibold text-gray-800">Items</h3><button onClick={handleAddItem} className={`text-sm font-medium ${theme.text} hover:bg-gray-50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1`}><Plus size={16} /> Add Item</button></div>
            ...
            <div className="space-y-2 flex-1">
              {items.map((item, index) => (
                <div key={item.id} className={`grid grid-cols-12 gap-4 items-center group product-row-wrapper ${item.isFree ? 'bg-amber-50/60 -mx-2 px-2 py-1.5 rounded-lg border-l-4 border-amber-400' : ''}`}>
                  <div className="col-span-5 relative">
                    <input 
                      ref={(el) => (itemRefs.current[`name-${item.id}`] = el)} 
                      type="text" 
                      placeholder="Item name"
                      value={item.name}
                      onFocus={() => !item.isFree && setFocusedRowId(item.id)}
                      onClick={() => !item.isFree && setFocusedRowId(item.id)}
                      onChange={(e) => handleUpdateItem(item.id, 'name', e.target.value)}
                      disabled={item.isFree}
                      className={`w-full p-2.5 bg-gray-50 rounded-lg text-sm border-2 border-transparent focus:bg-white focus:border-gray-200 outline-none transition-all ${item.isFree ? 'bg-transparent font-bold text-amber-900' : ''}`}
                    />
                    {item.isFree && <span className="absolute right-0 top-3 text-[10px] font-bold text-amber-600 bg-amber-100 px-2 rounded-full flex items-center gap-1"><Gift size={10}/> FREE</span>}
                    
                    {focusedRowId === item.id && !item.isFree && (
                       <div className="absolute top-full left-0 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-xl max-h-48 overflow-y-auto z-50">
                          {PRODUCTS.filter(p => p.name.toLowerCase().includes(item.name.toLowerCase())).map(p => (
                             <div key={p.id} onMouseDown={() => handleProductSelect(item.id, p)} className="px-3 py-2 hover:bg-gray-50 cursor-pointer text-sm text-gray-700 flex justify-between"><span>{p.name}</span><span className="text-gray-400 text-xs">₹{p.price}</span></div>
                          ))}
                       </div>
                    )}
                  </div>
                  <div className="col-span-2"><input ref={(el) => (itemRefs.current[`qty-${item.id}`] = el)} type="number" value={item.qty} onChange={(e) => handleUpdateItem(item.id, 'qty', parseFloat(e.target.value))} className={`w-full p-2.5 bg-gray-50 rounded-lg text-sm text-right border-2 border-transparent focus:bg-white focus:border-gray-200 outline-none ${item.isFree ? 'bg-white border-amber-200' : ''}`}/></div>
                  <div className="col-span-2"><input ref={(el) => (itemRefs.current[`price-${item.id}`] = el)} type="number" value={item.price} onChange={(e) => handleUpdateItem(item.id, 'price', parseFloat(e.target.value))} disabled={item.isFree} className={`w-full p-2.5 bg-gray-50 rounded-lg text-sm text-right border-2 border-transparent focus:bg-white focus:border-gray-200 outline-none ${item.isFree ? 'bg-white border-amber-200' : ''}`}/></div>
                  <div className="col-span-2 text-right font-bold text-gray-700">₹{item.amount.toLocaleString()}</div>
                  <div className="col-span-1 text-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleRemoveItem(item.id, item.isFree)} tabIndex={-1} className={`${item.isFree ? 'text-amber-400 hover:text-red-500' : 'text-gray-300 hover:text-red-500'} p-2 rounded-full hover:bg-red-50 transition-colors`}><Trash2 size={16} /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-6 pt-6 border-t border-gray-100 flex justify-end items-center gap-12"><span className="text-gray-400 text-sm">Total Items: {items.length}</span><div className="text-right"><span className="block text-xs text-gray-400 uppercase mb-1">Total Bill</span><span className="text-2xl font-bold text-gray-800">₹{totals.sub.toLocaleString()}</span></div></div>
          </div>
        </div>
